...
def test_all_features():
    print("🧪 Testing All Features of E-commerce Scraper")
    print("=" * 50)
    
    # Test 1: Basic scraping
    print("\n1️⃣ Testing Basic Scraping")
    ...
    print("\n4️⃣ Testing Summary Statistics")
    df = pd.DataFrame(products)
    print(f"✅ Price range: ${df['price'].astype(float).min():.2f} - ${df['price'].astype(float).max():.2f}")
    print(f"✅ Products with ratings: {len(df[df['rating'] != ''])}")
    ...
    try:
        ...
        print(f"✅ Handled invalid URL gracefully: {len(bad_products)} products")
